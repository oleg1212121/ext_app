...
class SentencesRelationManager extends RelationManager
{
        ...
        return $schema
            ->schema([
                Select::make('insert_after')
                    ->label('Insert after')
                    ->options(function (?EnEntitySentence $record, RelationManager $livewire): array {
                    ...
                    ->default(function (?EnEntitySentence $record, RelationManager $livewire): string {
                        $owner = $livewire->getOwnerRecord();
        ...
        return $table
            ...
            ->headerActions([
                Actions\CreateAction::make()
                    ->createAnother(false)
                    ->using(function (array $data, RelationManager $livewire): EnEntitySentence {
                        $owner = $livewire->getOwnerRecord();
                        $data['order'] = $this->computeOrder($owner, null, $data['insert_after']);
                        unset($data['insert_after']);

                        $sentence = new EnEntitySentence($data);
            ...
            ->recordActions([
                Actions\EditAction::make()
                    ->using(function (array $data, RelationManager $livewire, Model $record): EnEntitySentence {
                        $owner = $livewire->getOwnerRecord();
    ...
    private function computeOrder(EnEntity $owner, ?EnEntitySentence $excluding, string $insertAfterId): int
    {
        ...
        $service->rebalanceAll(EnEntitySentence::class, 'en_entity_id', $owner->id);
