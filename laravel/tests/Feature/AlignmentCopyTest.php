...
function storeMatch(Entity $first, Entity $second)
{
    return test()->actingAs(User::factory()->create())
        ->post("/library/{$first->work_id}/alignments", [
...
test('creating a match between exact copies reuses the completed alignment', function () {
    ...
    storeMatch($enCopy, $ruCopy)
        ->assertRedirect('/library/'.$work->id.'?tab=alignments')
...
test('a scrambled source order column does not survive the copy', function () {
    ...
    storeMatch($enCopy, $ruCopy)->assertRedirect('/library/'.$work->id.'?tab=alignments');
