<x-filament-panels::page>
    ...
    <div class="mb-4 rounded-lg bg-white p-4 shadow-sm dark:bg-gray-800">
        <div class="flex flex-wrap items-center gap-6">
            ...
            <div>
                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $this->sideLabel("a") }} Sentences</span>
                <span class="ml-2 font-semibold">{{ $run->a_total_sentences }}</span>
            </div>
            <div>
                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $this->sideLabel("b") }} Sentences</span>
                <span class="ml-2 font-semibold">{{ $run->b_total_sentences }}</span>
            </div>
            <div>
                ...
                <table class="w-full table-fixed border-collapse">
                    <thead>
                        <tr class="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900">
                            ...
                            <th class="w-12 px-2 py-3 text-center text-xs font-semibold text-gray-600 dark:text-gray-300">{{ $this->sideLabel("a") }}</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300">
                                {{ $this->sideLabel('a') }}
                            </th>
                            <th class="w-12 px-2 py-3 text-center text-xs font-semibold text-gray-600 dark:text-gray-300">{{ $this->sideLabel("b") }}</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300">
                                {{ $this->sideLabel('b') }}
                            </th>
                    ...
                    <tbody>
                        @foreach($rows as $index => $row)
                            @php
                                ...
                                } elseif ($row['type'] === 'skip_a') {
                                    $bgClass = 'bg-red-50 dark:bg-red-900/20';
                                } elseif ($row['type'] === 'skip_b') {
                            ...
                            <tr class="border-b border-gray-100 dark:border-gray-700 {{ $bgClass }} hover:brightness-95">
                                ...
                                <td class="px-4 py-2 text-sm text-gray-800 dark:text-gray-200">
                                    @forelse($row['a'] as $item)
                                        <div class="py-0.5">{{ \Illuminate\Support\Str::limit($item['content'], 300) }}</div>
                                    @empty
