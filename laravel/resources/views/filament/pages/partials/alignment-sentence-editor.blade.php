...
<div class="mb-2 w-full max-w-full min-w-0 rounded border border-gray-200 bg-white/60 p-2 dark:border-gray-600 dark:bg-gray-900/40">
    <textarea
        ...
        wire:change="updateSentenceContent('{{ $side }}', '{{ $sentence['key'] }}', $event.target.value)"
    >{{ $sentence['content'] }}</textarea>
    @if($showActions)
        <div class="flex flex-wrap gap-1">
            <x-filament::button
                wire:click="moveSentence('{{ $side }}', '{{ $sentence['key'] }}', 'up')"
            ...
            />
            <x-filament::button
                wire:click="moveSentence('{{ $side }}', '{{ $sentence['key'] }}', 'down')"
            ...
            @if($showUnlink)
                <x-filament::button
                    wire:click="unlinkSentence('{{ $side }}', '{{ $sentence['key'] }}')"
            ...
            @endif
            <x-filament::button
                wire:click="deleteSentence('{{ $side }}', '{{ $sentence['key'] }}')"
