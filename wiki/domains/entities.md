...
description: Inertia/React management area for creating and viewing language-keyed text entities grouped by work, driven by enabled languages.
...
approved user browse entities per enabled language (with a work filter), create
a new entity under an existing or inline-created work (with an optional text
file), inspect a single entity's detail, and — since ADR
...
The surface is a production consumer of the `Language` model — the
...
`List` (per-language list with a work column and an "All works" dropdown
filter + "+ Create entity"), `Create` (form with name, label, description,
file upload, and a work picker that either selects an existing work or creates
one inline: title, author, original language), `Show` (detail), `Edit`
(metadata form + dnd-kit sortable sentence manager). All use the `--wbench-*`
tokens to match the sibling Alignments management surface and the
...
The create form mirrors the Filament `EntityResource` behaviour: fields are
`name` (required), `label` (optional, distinguishes same-language variants of
a work), `description` (optional), `file` (optional `.txt`), and the work —
either an existing `work_id` or inline `new_work_title` / `new_work_author` /
`new_work_original_language_id` (defaults to the entity's own language when
omitted). On submit the controller resolves the work
(`EntityController::resolveWork`), stores the file to `entities/{lang}` on the
...
- **Match found** (≥0.95 cosine against an existing Entity in the same
  language): no new Entity is created. The uploader receives an Access grant on
  the existing Entity (with the match `similarity`), the uploaded file is
  deleted, and they are redirected to the existing Entity. This is how a user
  "uploads" a copyrighted work that already exists without creating a copy or
  infringing — they simply get linked.
...
accordingly, and reading an `EntityMatch` in the simulator requires grants
